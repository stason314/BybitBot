...
public static class FuturesOrderRequestFactory
{
    public static BybitCreateOrderRequest Create(FuturesTradeIntent intent)
    {
        ...
        return new BybitCreateOrderRequest
        {
            ...
            TriggerPrice = intent.TriggerPrice is { } triggerPrice ? FormatDecimal(triggerPrice) : null,
            TriggerDirection = intent.TriggerDirection,
            TriggerBy = intent.TriggerPrice is null ? null : intent.TriggerBy ?? "MarkPrice",
            TimeInForce = intent.OrderType == OrderType.Limit || intent.TriggerPrice is not null ? "GTC" : "IOC",
            IsLeverage = 0,
