...
Sparse crash shorts can use a separate gate instead of the normal rolling walk-forward pass:
`TURTLE_CRASH_SHORT_GATE_ENABLED=true` allows Turtle Short S2 only while BTC is in a configured crash regime,
and the backtest gate can admit profitable Short S2 symbols when the OOS crash profile passes
...
TURTLE_CRASH_SHORT_SIZE_MULTIPLIER=1
